...
(function($) {
	...
	var general = {
		init: function() {
			...
			/*$.wfactory_439.hooks.add('core/components/updated', function(button, data, response) {
				// Выполняем код ниже, только на страницах плагина с интерфейсом Clearfy
			...
			$('.wbcr-clearfy-popup-button-ok').click(function() {
				...
				self.sendRequest({
						...
						if( !data || data.error ) {
							 ...
							 * $.wbcr_factory_clearfy_230.hooks.add('wbcr/factory_clearfy_230/updated',
							 * function(noticeId) {});
							 * @param {string} modeName - имя режима конфигурации
							 * @param {object} data
							 */

							$.wbcr_factory_clearfy_230.hooks.run('clearfy/quick_start/configurated_error', [
								modeName,
								data
							]);
							return;
					...
					function(data) {
						 ...
						 * libs/clearfy/admin/assests/js/global.js Пример регистрации хука
						 * $.wbcr_factory_clearfy_230.hooks.add('wbcr/factory_clearfy_230/updated', function(noticeId)
						 * {});
						 * @param {string} modeName - имя режима конфигурации
						 * @param {object} data
						 */
						$.wbcr_factory_clearfy_230.hooks.run('clearfy/quick_start/configurated', [modeName, data]);
		...
		importOptions: function() {
			...
			$('.wbcr-clearfy-import-options-button').click(function() {
				...
				if( void 0 === wbcr_clearfy_ajax || !wbcr_clearfy_ajax.import_options_nonce ) {
					$.wbcr_factory_clearfy_230.app.showNotice('Unknown Javascript error, most likely the wbcr_clearfy_ajax variable does not exist!', 'danger');
					return false;
				}
			...
			$.ajax(ajaxurl, {
				...
				success: function(data, textStatus, jqXHR) {
					...
					if( !data || data.error ) {
						...
						if( data ) {
							...
							if( void 0 !== wbcr_clearfy_ajax ) {
								noticeId = $.wbcr_factory_clearfy_230.app.showNotice(wbcr_clearfy_ajax.i18n.unknown_error, 'danger');
							}
						}
					...
					if( !request_data.flush_redirect ) {
						if( void 0 !== wbcr_clearfy_ajax ) {
							noticeId = $.wbcr_factory_clearfy_230.app.showNotice(wbcr_clearfy_ajax.i18n.success_update_settings, 'success');
